...
async function run() {
    try {
        ...
        const FeaturesTextCollection = client.db('toyhouse').collection('FeaturesTextCollection');
        const FeaturesItemCollection = client.db('toyhouse').collection('FeaturesItemCollection');
        const TeammembersTextCollection = client.db('toyhouse').collection('TeammembersTextCollection');
        const TeammembersItemsCollection = client.db('toyhouse').collection('TeammembersItemsCollection');
        const ExperienceCollection = client.db('toyhouse').collection('ExperienceCollection');
        ...
        app.post('/add-experience', async (req, res) => {
            ...
            const result = await ExperienceCollection.insertOne(addexperience);
            res.send(result);
        });
        ...
        app.get('/experiences', async (req, res) => {
            ...
            const experiencese = await cursor.toArray();
            res.send(experiencese);
        });
        ...
        app.put("/update-experiences/:id", async (req, res) => {
            const id = req.params.id;
            const sectionExperience = req.body;
            const filter = { _id: new ObjectId(id) };
            const options = { upsert: true };
            const updatedDoc = {
                $set: {

                    experienceText: sectionExperience.experienceText,
                    experienceHeading: sectionExperience.experienceHeading,
                    experienceDetails: sectionExperience.experienceDetails,
                    skillOneText: sectionExperience.skillOneText,
                    skillOneNumber: sectionExperience.skillOneNumber,
                    skillTwoText: sectionExperience.skillTwoText,
                    skillTwoNumber: sectionExperience.skillTwoNumber,
                    skillThreeText: sectionExperience.skillThreeText,
                    skillThreeNumber: sectionExperience.skillThreeNumber,
                },
            };
        ...
        app.post('/add-teammember-text', async (req, res) => {
            const addTeam = req.body;
            const result = await TeammembersTextCollection.insertOne(addTeam);
            res.send(result);
        });

        app.get('/teamstext', async (req, res) => {
            const query = {};
            const cursor = TeammembersTextCollection.find(query);
            const teamtext = await cursor.toArray();
            res.send(teamtext);
        });


        app.get('/teamstext/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: ObjectId(id) };
            const team = await TeammembersTextCollection.findOne(query);
            res.send(team);

        });


        app.put("/teamstext/:id", async (req, res) => {
            const id = req.params.id;
            const teamTextSection = req.body;
            const filter = { _id: new ObjectId(id) };
            const options = { upsert: true };
            const updatedDoc = {
                $set: {
                    teamText: teamTextSection.teamText,
                    teamHeading: teamTextSection.teamHeading,


                },
            };

            const result = await TeammembersTextCollection.updateOne(
                filter,
                updatedDoc,
                options
            );
        ...
        /*** Team Member Section Item Satrt* **/


        app.post('/add-team-Item', async (req, res) => {
            const addMemberItem = req.body;
            const result = await TeammembersItemsCollection.insertOne(addMemberItem);
            res.send(result);
        });

        app.get('/teamitems', async (req, res) => {
            const query = {};
            const cursor = TeammembersItemsCollection.find(query);
            const memberitems = await cursor.toArray();
            res.send(memberitems);
        });

        app.get('/teamitems/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: ObjectId(id) };
            const memberitem = await TeammembersItemsCollection.findOne(query);
            res.send(memberitem);

        });


        app.put("/update-teamitems/:id", async (req, res) => {
            const id = req.params.id;
            const memberSection = req.body;
            const filter = { _id: new ObjectId(id) };
            const options = { upsert: true };
            const updatedDoc = {
                $set: {
            ...
            const result = await TeammembersItemsCollection.updateOne(
                filter,
                updatedDoc,
                options
            );
        ...
        /*** Start Features Service Section Start * **/


        app.post('/add-feature-text', async (req, res) => {
            const addfeature = req.body;
            const result = await FeaturesTextCollection.insertOne(addfeature);
            res.send(result);
        });

        app.get('/featurestext', async (req, res) => {
            const query = {};
            const cursor = FeaturesTextCollection.find(query);
            const features = await cursor.toArray();
            res.send(features);
        });


        app.get('/featurestext/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: ObjectId(id) };
            const future = await FeaturesTextCollection.findOne(query);
            res.send(future);

        });


        app.put("/featurestext/:id", async (req, res) => {
            const id = req.params.id;
            const featuresTextSection = req.body;
            const filter = { _id: new ObjectId(id) };
            const options = { upsert: true };
            const updatedDoc = {
                $set: {
                    FeaturesText: featuresTextSection.FeaturesText,
                    FeaturesHeading: featuresTextSection.FeaturesHeading,
            ...
            const result = await FeaturesTextCollection.updateOne(
                filter,
                updatedDoc,
                options
            );
        ...
        /*** Features Service Section Item Satrt* **/


        app.post('/add-feature-Item', async (req, res) => {
            const addFutureItem = req.body;
            const result = await FeaturesItemCollection.insertOne(addFutureItem);
            res.send(result);
        });

        app.get('/featureitems', async (req, res) => {
            const query = {};
            const cursor = FeaturesItemCollection.find(query);
            const featureitems = await cursor.toArray();
            res.send(featureitems);
        });

        app.get('/featureitems/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: ObjectId(id) };
            const featureitem = await FeaturesItemCollection.findOne(query);
            res.send(featureitem);

        });


        app.put("/update-featureitems/:id", async (req, res) => {
            const id = req.params.id;
            const featuresSection = req.body;
            const filter = { _id: new ObjectId(id) };
            const options = { upsert: true };
            const updatedDoc = {
                $set: {
                    FeaturesImg: featuresSection.FeaturesImg,
                    FeaturesIcon: featuresSection.FeaturesIcon,
                    FeaturesText: featuresSection.FeaturesText,
                    FeaturesDesc: featuresSection.FeaturesDesc,


                },
            };

            const result = await FeaturesItemCollection.updateOne(
                filter,
                updatedDoc,
                options
            );
        ...
        app.put("/update-banner/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    ...
                    bannerText: bannerSection.bannerText,
                    imageOne: bannerSection.imageOne,
                    buttonText: bannerSection.buttonText,
                    buttonURL: bannerSection.buttonURL,
 ...
 * **/
        ...
        app.put("/update-worksec/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    stepNumber: workSection.stepNumber,
 ...
 * **/
        ...
        app.put("/update-counters/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    ...
                    counterSectionImg: counterSection.counterSectionImg,
                    counterSectionImgTwo: counterSection.counterSectionImgTwo,
                    counterSectionText: counterSection.counterSectionText,
                    counterSectionHeading: counterSection.counterSectionHeading,
                    counterSectionDec: counterSection.counterSectionDec,
        ...
        app.put("/update-videos/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    headingVideo: videoSection.headingVideo,
                    videoImage: videoSection.videoImage,
        ...
        app.put("/videos-items/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    videoItemIcon: itemSection.videoItemIcon,
                    videoItemTitle: itemSection.videoItemTitle,
                    videoItemDetails: itemSection.videoItemDetails,
        ...
        app.put("/footer-address/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    ...
                    phoneNumber: contactSection.phoneNumber,
...
app.get('/', (req, res) => {
    res.send('Good Job, SEO-12 Server is Working Fine');
});

app.listen(port, () => {
    console.log('Good Job, SEO-12  Server is Live Now', port);
});